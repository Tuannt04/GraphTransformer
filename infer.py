...
def add_arguments(parser):
    ...
    parser.add_argument("--from_vocab", type=str, default="data/cvocab",
                        help="from vocab path")
    parser.add_argument("--to_vocab", type=str, default="data/evocab",
                        help="to vocab path")
    ...
    parser.add_argument("--enc_layers", type=int, default=4, help="Number of layers in the encoder") # SỬA TỪ 6 (hoặc 10) sang 4
    parser.add_argument("--dec_layers", type=int, default=4, help="Number of layers in the decoder") # SỬA TỪ 6 (hoặc 10) sang 4
    parser.add_argument("--num_units", type=int, default=256, help="Size of each model layer") # SỬA TỪ 512 sang 256
    parser.add_argument("--num_heads", type=int, default=4, help="Size of each model layer") # SỬA TỪ 8 (hoặc 2) sang 4
    parser.add_argument("--emb_dim", type=int, default=300, help="Dimension of word embedding") # SỬA TỪ 300 sang 256
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size to use during training") # SỬA TỪ 128 sang 16
    ...
    parser.add_argument("--use_copy", type=int, default=0, help="Whether use copy mechanism")
    ...
    with tf.gfile.GFile(src_path, mode="r") as src_file:
        with tf.gfile.GFile(edge_path, mode="r") as edge_file:
            with tf.gfile.GFile(ref_path, mode="r") as ref_file:
                src, edges_str, ref = src_file.readline(), edge_file.readline(), ref_file.readline()
                
                while src and ref:
                    try:
                        ct += 1
                        # [FIX] Xử lý dữ liệu trong khối Try để bắt lỗi
                        src_seq = src.lower().rstrip("\n").split(" ")
                        tgt = ref.lower().rstrip("\n").split(" ")
                        ref_txt = ref.rstrip("\n")
                        
                        # Parse JSON đồ thị
                        graph = json.loads(edges_str.rstrip("\n"))
                        
                        src_ids = []
                        tgt_ids = []
                        char_ids = []
                        unk = []
                        edges_list = [] # Đổi tên biến để tránh trùng với edges_str
                        ...
                        for l in graph:
                            ...
                            for pair in graph[l]:
                                ...
                                # [FIX] Nếu gặp lỗi input graph, raise lỗi để bỏ qua dòng này
                                if depth[id1] == []:
                                    #raise ValueError("input graph error: depth[id1] is empty")
                                    pass
                                ...
                                edges_list.append([edge, id1, int(id2)])

                        data_set.append([src_ids, tgt_ids, edges_list, char_ids, depth, ref_txt])
                        unks.append(unk)
                        if not(len(src_ids) < hparams.max_src_len - 1 and len(edges_list) < hparams.max_src_len - 1 and len(
                                tgt_ids) < hparams.max_tgt_len - 1):
                            lct += 1
                            
                    except Exception as e:
                        # In lỗi và bỏ qua, không làm chết chương trình
                        print("Bỏ qua lỗi dữ liệu tại dòng {}: {}".format(ct, e))

                    # [QUAN TRỌNG] Đọc dòng tiếp theo (Nằm ngoài try-except)
                    src, edges_str, ref = src_file.readline(), edge_file.readline(), ref_file.readline()
...
def infer(hparams):
    print("Bắt đầu infer...")
    import time # Cần thư viện time để bấm giờ

    # [FIX CẤP TỐC] Ép cứng batch_size = 1 để theo dõi tiến độ trên CPU.
    # Code gốc của bạn không có batch_size ở chế độ infer, nên ta set_hparam tại đây.
    ...
    # Bỏ qua lỗi đồ thị để chương trình chạy mượt
    train_data, train_unks = read_data_graph("data/train.src", "data/train.edge", "data/train.tgt", wvocab, evocab, cvocab, hparams)
    valid_data, valid_unks = read_data_graph("data/valid.src", "data/valid.edge", "data/valid.tgt", wvocab, evocab, cvocab, hparams)
    test_data, test_unks = read_data_graph("data/test.src", "data/test.edge", "data/test.tgt", wvocab, evocab, cvocab, hparams)
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        try:
            ...
            print("Chạy với model untrained.")
    else:
        print("Không có checkpoint, chạy với model untrained.")
    ...
    total_batches = int(total_test_data / batch_size) + 1
    
    print("\n--- Bắt đầu suy luận (Test Data: {} câu) ---".format(total_test_data))

    for id in range(0, total_batches):
        
        # [THÊM] Bấm giờ và in tiến độ
        start_time = time.time()
        print("Batch: {} / {} (Đã xong: {})".format(id + 1, total_batches, ct))
        ...
        if batch_size > 0:
             # Vì batch_size=1, nên duration chính là thời gian/câu
             print("   -> Thời gian/câu: {:.2f} giay.".format(duration))
        ...
        for i in range(batch_size):
            ...
            if idx >= total_test_data: # Đảm bảo không vượt quá chỉ mục cuối cùng
                break
                
            _, _, _, _, _, answer = test_data[idx]
            unks = test_unks[idx]
            f1.write(answer + "\n")
            ...
            for output in sample_output:
                ...
                if w == "_UNK":
                    if unk < len(align[i]) and align[i][unk] < len(unks):
                        y = unks[align[i][unk]]
                        outputs.append(y)
                    elif unk < len(unks): # Xử lý trường hợp align bị thiếu
                        outputs.append(unks[unk])
                    else: # Nếu không tìm thấy unk, dùng UNK
                        outputs.append("_UNK") 
                elif w != "_UNK":
                    outputs.append(w)
                unk += 1
            ...
            ct += 1
            if ct >= total_test_data:
                break
        
        # [THÊM] Ép ghi file xuống ổ cứng ngay sau mỗi câu
        f1.flush()
        f2.flush()
...
def init_embedding(hparams):
    ...
    emb_list = []
    embedding_dim = hparams.emb_dim # Kích thước mong đợi: 1024
    
    for i in range(0, len(vocab)):
        word = vocab[i]
        
        # 1. Khởi tạo vector ngẫu nhiên an toàn
        random_vector = (0.1 * np.random.random([embedding_dim]) - 0.05).astype(np.float32)

        if word in word_vectors:
            vector = word_vectors[word]
            
            # [FIX KIỂM TRA KÍCH THƯỚC]
            if vector.shape[0] == embedding_dim:
                emb_list.append(vector.astype(np.float32))
            else:
                # Nếu kích thước không khớp, dùng vector ngẫu nhiên. (Sử dụng cú pháp format())
                print("Cảnh báo: Kích thước vector cho '{}' là {}, dự kiến {}. Dùng ngẫu nhiên.".format(word, vector.shape[0], embedding_dim))
                emb_list.append(random_vector)
        else:
            # Dùng vector ngẫu nhiên
            emb_list.append(random_vector)

    print(" init embedding finished")
    
    # [FIX CỐT LÕI]: Sử dụng np.stack để đảm bảo mảng 2D đồng nhất
    try:
        final_embedding = np.stack(emb_list, axis=0)
    except ValueError as e:
        # Sửa lỗi cú pháp f-string thành format()
        print("LỖI TẠO MẢNG: Kích thước các vector không đồng nhất. Chi tiết: {}".format(e))
        raise
...
def main(_):
    ...
    # SỬA LẠI CHO KHỚP VỚI TRAIN.PY:
    # Dùng giá trị từ FLAGS thay vì hardcode
    print("--- Thông số mô hình ---")
    print("enc_layers:", hparams.enc_layers)
    print("dec_layers:", hparams.dec_layers)
    print("num_units:", hparams.num_units)
    print("num_heads:", hparams.num_heads)
    print("emb_dim:", hparams.emb_dim)
    infer(hparams)
